...
class Carnival {
  ...
  createScene() {
    ...
    let camera: ArcRotateCamera | FreeCamera | null;
    if (this.mode === 'free') {
      camera = new FreeCamera('camera', new Vector3(0, this.viewY, -5), scene);
      camera.setTarget(Vector3.Zero());
      camera.rotation = new Vector3(Tools.ToRadians(-2.5), Tools.ToRadians(-175), 0);
    } else {
    ...
    this.camera = camera;
    scene.environmentTexture = new CubeTexture(skyEnv, scene);
    ...
    skyboxMaterial.reflectionTexture = new CubeTexture(skyUrl, scene);
    skyboxMaterial.reflectionTexture.coordinatesMode = Texture.SKYBOX_MODE;
    skyboxMaterial.disableLighting = true;
    skybox.material = skyboxMaterial;

    // 导入场景
    SceneLoader.LoadAssetContainer(model, '', scene, (container) => {
      console.log(container);
      container.instantiateModelsToScene((name) => {
        container.meshes.forEach((mesh) => {
          mesh.checkCollisions = true;
          if (mesh.name === 'BigScreen') {
            this.addTexture(mesh as Mesh);
          }
        });
        return name;
      });
  ...
  moveCamera() {
    ...
    if (inputMap['w']) {
      camera.position.addInPlace(camera.getDirection(Axis.Z).scale(cameraSpeed));
    }
    if (inputMap['s']) {
      camera.position.subtractInPlace(camera.getDirection(Axis.Z).scale(cameraSpeed));
    }
    if (inputMap['a']) {
      camera.position.addInPlace(camera.getDirection(Axis.X).scale(-cameraSpeed));
    }
    if (inputMap['d']) {
      camera.position.addInPlace(camera.getDirection(Axis.X).scale(cameraSpeed));
    }
    ...
    camera.setTarget(camera.position.add(camera.getDirection(Axis.Z)));
    camera.position.y = this.viewY; // set the camera box position to a fixed height
  }

  addTexture(mesh: Mesh) {
    ...
    texture.vScale = -1;
    const mat = new StandardMaterial('bigScreen', this.scene);
    mat.freeze();
    ...
    scene.onPointerUp = function (evt) {
      ...
      if (pickResult?.hit) {
        ...
        if (absX < min && absY < min && absZ < min) {
          that.moving = true;
          const newTarget = new Vector3(curP?.x, that.viewY, curP?.z);
